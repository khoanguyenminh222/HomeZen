import { z } from 'zod';

// Validation cho số điện thoại Việt Nam (10-11 chữ số)
const phoneRegex = /^(0[3|5|7|8|9])+([0-9]{8,9})$/;

// Validation cho CMND (9 chữ số) hoặc CCCD (12 chữ số)
const idCardRegex = /^(\d{9}|\d{12})$/;

// Schema cho tạo người thuê mới
export const createTenantSchema = z.object({
  ho_ten: z.string()
    .min(1, 'Họ tên không được để trống')
    .max(100, 'Họ tên không được quá 100 ký tự'),

  dien_thoai: z.string()
    .regex(phoneRegex, 'Số điện thoại không đúng định dạng (VD: 0901234567)'),

  can_cuoc: z.string()
    .regex(idCardRegex, 'CMND/CCCD phải có 9 hoặc 12 chữ số')
    .optional()
    .or(z.literal(''))
    .nullable(),

  ngay_sinh: z.string()
    .optional()
    .nullable()
    .transform((val) => val ? new Date(val) : undefined),

  que_quan: z.string()
    .max(200, 'Quê quán không được quá 200 ký tự')
    .optional()
    .or(z.literal(''))
    .nullable(),

  ngay_vao_o: z.string()
    .optional()
    .nullable()
    .transform((val) => val ? new Date(val) : undefined),

  tien_coc: z.union([z.string(), z.number()])
    .optional()
    .nullable()
    .transform((val) => {
      if (val === null || val === undefined || val === '') return null;
      const num = typeof val === 'number' ? val : parseFloat(val);
      return isNaN(num) || num < 0 ? null : num;
    }),

  phong_id: z.string()
    .optional()
    .nullable(),

  url_hop_dong: z.string()
    .url('URL file hợp đồng không hợp lệ')
    .optional()
    .or(z.literal(''))
    .nullable(),

  // Thông tin bổ sung
  gioi_tinh: z.string()
    .max(50, 'Giới tính không được quá 50 ký tự')
    .optional()
    .or(z.literal(''))
    .nullable(),

  nghe_nghiep: z.string()
    .max(200, 'Nghề nghiệp không được quá 200 ký tự')
    .optional()
    .or(z.literal(''))
    .nullable(),

  dan_toc: z.string()
    .max(100, 'Dân tộc không được quá 100 ký tự')
    .optional()
    .or(z.literal(''))
    .nullable(),

  quoc_tich: z.string()
    .max(100, 'Quốc tịch không được quá 100 ký tự')
    .optional()
    .or(z.literal(''))
    .nullable(),

  dia_chi_thuong_tru: z.string()
    .max(500, 'Địa chỉ thường trú không được quá 500 ký tự')
    .optional()
    .or(z.literal(''))
    .nullable(),

  dia_chi_tam_tru: z.string()
    .max(500, 'Địa chỉ tạm trú không được quá 500 ký tự')
    .optional()
    .or(z.literal(''))
    .nullable(),

  so_the_bao_hiem: z.string()
    .max(50, 'Số thẻ bảo hiểm không được quá 50 ký tự')
    .optional()
    .or(z.literal(''))
    .nullable(),

  ngay_cap: z.string()
    .optional()
    .nullable()
    .transform((val) => val ? new Date(val) : undefined),

  noi_cap: z.string()
    .max(200, 'Nơi cấp không được quá 200 ký tự')
    .optional()
    .or(z.literal(''))
    .nullable()
});

// Schema cho cập nhật người thuê
export const updateTenantSchema = createTenantSchema.partial();

// Schema cho tạo người ở
export const createOccupantSchema = z.object({
  ho_ten: z.string()
    .min(1, 'Họ tên không được để trống')
    .max(100, 'Họ tên không được quá 100 ký tự'),

  can_cuoc: z.string()
    .regex(idCardRegex, 'CMND/CCCD phải có 9 hoặc 12 chữ số')
    .optional()
    .or(z.literal(''))
    .nullable(),

  ngay_sinh: z.string()
    .optional()
    .nullable()
    .transform((val) => val ? new Date(val) : undefined),

  que_quan: z.string()
    .max(200, 'Quê quán không được quá 200 ký tự')
    .optional()
    .or(z.literal(''))
    .nullable(),

  moi_quan_he: z.string()
    .max(100, 'Quan hệ không được quá 100 ký tự')
    .optional()
    .or(z.literal(''))
    .nullable(),

  loai_cu_tru: z.enum(['TAM_TRU', 'THUONG_TRU'], {
    errorMap: () => ({ message: 'Loại cư trú phải là "Tạm trú" hoặc "Thường trú"' })
  }).default('TAM_TRU'),

  // Thông tin bổ sung
  dien_thoai: z.string()
    .regex(phoneRegex, 'Số điện thoại không đúng định dạng (VD: 0901234567)')
    .optional()
    .or(z.literal(''))
    .nullable(),

  gioi_tinh: z.string()
    .max(50, 'Giới tính không được quá 50 ký tự')
    .optional()
    .or(z.literal(''))
    .nullable(),

  nghe_nghiep: z.string()
    .max(200, 'Nghề nghiệp không được quá 200 ký tự')
    .optional()
    .or(z.literal(''))
    .nullable(),

  dan_toc: z.string()
    .max(100, 'Dân tộc không được quá 100 ký tự')
    .optional()
    .or(z.literal(''))
    .nullable(),

  quoc_tich: z.string()
    .max(100, 'Quốc tịch không được quá 100 ký tự')
    .optional()
    .or(z.literal(''))
    .nullable(),

  dia_chi_thuong_tru: z.string()
    .max(500, 'Địa chỉ thường trú không được quá 500 ký tự')
    .optional()
    .or(z.literal(''))
    .nullable(),

  dia_chi_tam_tru: z.string()
    .max(500, 'Địa chỉ tạm trú không được quá 500 ký tự')
    .optional()
    .or(z.literal(''))
    .nullable(),

  so_the_bao_hiem: z.string()
    .max(50, 'Số thẻ bảo hiểm không được quá 50 ký tự')
    .optional()
    .or(z.literal(''))
    .nullable(),

  ngay_cap: z.string()
    .optional()
    .nullable()
    .transform((val) => val ? new Date(val) : undefined),

  noi_cap: z.string()
    .max(200, 'Nơi cấp không được quá 200 ký tự')
    .optional()
    .or(z.literal(''))
    .nullable()
});

// Schema cho cập nhật người ở
export const updateOccupantSchema = createOccupantSchema.partial();

// Schema cho hoàn trả cọc
export const depositReturnSchema = z.object({
  so_tien: z.number()
    .min(0, 'Số tiền hoàn trả phải >= 0'),

  loai_hoan_tra: z.enum(['HOAN_TRA_DAY_DU', 'TRU_VAO_HOA_DON_CUOI'], {
    errorMap: () => ({ message: 'Phương thức hoàn trả không hợp lệ' })
  }),

  ghi_chu: z.string()
    .max(500, 'Ghi chú không được quá 500 ký tự')
    .optional()
    .or(z.literal(''))
    .nullable()
});

// Schema cho chuyển người thuê chính
export const transferTenantSchema = z.object({
  nguoi_thue_moi_id: z.string()
    .min(1, 'Phải chọn người thuê mới'),

  phong_id: z.string()
    .min(1, 'Phải chọn phòng')
});
