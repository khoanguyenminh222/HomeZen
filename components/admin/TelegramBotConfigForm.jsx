    ...
    async function fetchBotConfig() {
      try {
        ...
        const response = await fetch(
          "/api/admin/telegram-bot-config?decrypted=true",
        );
        if (response.ok) {
    ...
    try {
      ...
      if (response.ok && result.success) {
        ...
        if (result.botInfo?.username) {
          const ten_bot = result.botInfo.username.startsWith("@")
            ? result.botInfo.username
            : `@${result.botInfo.username}`;
          setValue("ten_bot", ten_bot);
        }
      } else {
        toast({
          title: "Lỗi",
      ...
      if (response.ok) {
        ...
        if (refreshResponse.ok) {
          ...
          if (refreshResult.data) {
            setValue("bot_token", refreshResult.data.bot_token || "");
            setValue("ten_bot", refreshResult.data.ten_bot || "");
          }
        }
      } else {
        toast({
      ...
      <Card>
        <CardHeader className="pb-4 sm:pb-6">
          ...
          <CardDescription className="text-sm sm:text-base mt-2">
            Cấu hình bot token global cho tất cả property owners. Bot token này
            sẽ được dùng chung cho tất cả người dùng.
          </CardDescription>
        </CardHeader>
        <CardContent className="pt-0">
          ...
          >
            ...
            <div className="space-y-2">
              ...
              {errors.ten_bot && (
                <p className="text-xs sm:text-sm text-red-500">
                  {errors.ten_bot.message}
                </p>
              )}
              <p className="text-xs sm:text-sm text-muted-foreground">
                Username của bot để gợi ý cho người dùng. Sẽ tự động lấy từ
                Telegram API khi test bot token.
              </p>
            </div>
            ...
            <div className="flex gap-3">
              <Button
                ...
                disabled={isTesting || isLoading || !bot_token}
