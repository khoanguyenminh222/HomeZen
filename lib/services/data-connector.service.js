...
export class DataConnectorService {
    ...
    async executeProcedure(procedureName, parameters, schemaParams = [], limit = null) {
        try {
            ...
            // 1. Chuẩn bị giá trị tham số và Mapping kiểu dữ liệu Postgres
            const typeMapping = {
                'integer': 'integer',
                'int4': 'integer',
                'int8': 'bigint',
            ...
            const values = schemaParams.map((p) => {
                let val = parameters[p.name];

                if (p.name === 'p_uid' && (val === undefined || val === null)) {
                    val = parameters.p_uid || null;
                }

                if (!val && val !== 0 && val !== false) return null;

                const lowerType = p.type.toLowerCase();
                if (lowerType.includes('int')) return parseInt(val);
                if (lowerType.includes('decimal') || lowerType.includes('numeric') || lowerType.includes('float')) return parseFloat(val);
                if (lowerType.includes('bool')) return val === 'true' || val === true;

                return val;
            });

            // 2. Tạo câu lệnh SQL động với Ép kiểu tường minh (Explicit Casting)
            const paramPlaceholders = schemaParams.map((p, i) => {
                const baseType = Object.keys(typeMapping).find(k => p.type.toLowerCase().includes(k));
                const pgType = baseType ? typeMapping[baseType] : 'text';
                return `$${i + 1}::${pgType}`;
            }).join(', ');
