    ...
    return (
        <div className="space-y-4">
            <div className="flex flex-col sm:flex-row gap-2 mb-4">
                ...
                <Button onClick={fetchDeletedTenants} className="h-10 w-full sm:w-24">Tìm kiếm</Button>
            </div>
                            ...
                            <CardHeader className="pb-2">
                                <div className="flex flex-col sm:flex-row justify-between items-start gap-2">
                                    <div className="flex-1 min-w-0">
                                        <CardTitle className="text-lg wrap-break-word">{tenant.fullName}</CardTitle>
                                        <p className="text-sm text-muted-foreground wrap-break-word">{tenant.phone}</p>
                                    </div>
                                    <Badge variant="outline" className="text-red-600 border-red-200 w-fit">Đã xóa</Badge>
                                </div>
                            </CardHeader>
                            <CardContent className="space-y-4">
                                <div className="grid grid-cols-1 gap-2 text-sm">
                                    <div className="flex items-center gap-2">
                                        ...
                                        <span>Xóa ngày: {formatDate(tenant.deletedAt)}</span>
                                    </div>
                                    <div className="flex items-center gap-2">
                                        <UserIcon className="h-4 w-4 text-muted-foreground" />
                                        <span>Người xóa: {tenant.deletedBy || 'Hệ thống'}</span>
                                    </div>
                                    <div className="flex items-start gap-2">
                                        <MessageSquare className="h-4 w-4 text-muted-foreground mt-0.5" />
                                        <span className="italic">Lý do: {tenant.deleteReason || 'N/A'}</span>
                                    </div>
                                </div>
                                        ...
                                        <AlertDialogContent className="w-[95vw] sm:w-full max-w-md">
                                            <AlertDialogHeader>
                                                ...
                                                <AlertDialogDescription className="wrap-break-word">
                                                    Hành động này sẽ xóa <strong>VĨNH VIỄN</strong> hồ sơ người thuê <strong>{tenant.fullName}</strong>.
                                                    Tất cả dữ liệu liên quan sẽ biến mất và <strong>KHÔNG THỂ</strong> khôi phục lại.
                                                </AlertDialogDescription>
                                            </AlertDialogHeader>
