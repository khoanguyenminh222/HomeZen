...
export class DataConnectorService {
    ...
    async executeProcedure(procedureName, parameters, schemaParams = [], limit = null) {
        try {
            // 0. Lấy thông tin Procedure từ DB để biết returnType (table/procedure/void)
            const procedure = await prisma.reportProcedure.findUnique({
                where: { name: procedureName },
                select: { returnType: true }
            });
            ...
            let sql = '';
            const isProcedure = procedure?.returnType === 'procedure';
            ...
            throw new ReportError(
                `Lỗi khi truy xuất dữ liệu từ procedure "${procedureName}": ${this.mapDatabaseError(error)}`,
