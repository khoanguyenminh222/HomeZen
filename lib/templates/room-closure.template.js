...
export function formatRoomClosureTelegramMessage(propertyName, rooms, closureDay, daysRemaining) {
  ...
  if (rooms.length === 1) {
    ...
    message += `   • Phòng: <b>${room.ma_phong}</b> - ${room.ten_phong}\n`;
    message += `   • Người thuê: ${room.ten_nguoi_thue}\n`;
    message += `   • SĐT: ${room.so_dien_thoai_nguoi_thue}\n`;
  } else {
    message += `📋 <b>Danh sách phòng cần chốt sổ (${rooms.length} phòng):</b>\n\n`;

    rooms.forEach((room, index) => {
      message += `${index + 1}. <b>${room.ma_phong}</b> - ${room.ten_phong}\n`;
      message += `   👤 ${room.ten_nguoi_thue} | 📞 ${room.so_dien_thoai_nguoi_thue}\n`;
