...
import { getParameterManager } from './report/parameter-manager.service';
import { ReportErrorHandler } from './report/report-error-handler';
...
export class ReportGeneratorService {
    constructor() {
        ...
        this.parameterManager = getParameterManager();
    ...
    async generateReport({ templateId, parameters, userId }) {
        ...
        try {
            ...
            // 2. Xử lý tham số động (Phase 2 Integration)
            const config = this.parameterManager.parseConfiguration(template.anh_xa_tham_so);
            const processingResult = await this.parameterManager.processParameters(parameters || {}, config);

            if (!processingResult.success) {
                throw new ReportError(
                    ReportErrorHandler.formatValidationErrors(processingResult.errors),
                    ReportErrorCode.LOI_THAM_SO_KHONG_HOP_LE || 'VALIDATION_ERROR',
                    ReportErrorCategory.XUAT_BAO_CAO
                );
            }

            const transformedParameters = processingResult.data;

            // 2.1 Làm giàu tham số: Tự động tiêm p_uid nếu procedure yêu cầu
            const enrichedParameters = { ...transformedParameters };
            ...
            reportLogger.info(`Report (PDF) generated successfully in ${generationTime}ms (direct buffer)`);
    ...
    async generatePreview({ templateId, parameters, userId }) {
        try {
            ...
            if (!processingResult.success) {
                throw new Error(ReportErrorHandler.formatValidationErrors(processingResult.errors));
            }

            const transformedParameters = processingResult.data;

            // Làm giàu tham số: Tự động tiêm p_uid nếu procedure yêu cầu
            const enrichedParameters = { ...transformedParameters };
            if (userId && template.thu_tuc.tham_so?.some(p => p.name === 'p_uid')) {
                enrichedParameters.p_uid = userId;
            }
