...
export function isSuperAdmin(session) {
  return session?.user?.vai_tro === 'SIEU_QUAN_TRI';
}
...
export function isPropertyOwner(session) {
  return session?.user?.vai_tro === 'CHU_NHA_TRO';
}
...
export async function validateResourceOwnership(userId, resourceId, resourceType) {
  ...
  switch (resourceType) {
    case 'room':
      resource = await prisma.pRP_PHONG.findUnique({
        where: { id: resourceId },
        select: { nguoi_dung_id: true }
      });
      break;
    case 'feeType':
      resource = await prisma.bIL_LOAI_PHI.findUnique({
        where: { id: resourceId },
        select: { nguoi_dung_id: true }
      });
      break;
    case 'utilityRate':
      resource = await prisma.pRP_DON_GIA_DIEN_NUOC.findUnique({
        where: { id: resourceId },
        select: { nguoi_dung_id: true }
      });
      break;
  ...
  if (!resource || !resource.nguoi_dung_id) {
    return false;
  }

  // Property owners can only access their own resources
  return resource.nguoi_dung_id === userId;
}
...
export function requireAuth(handler) {
  return async (request, context) => {
    ...
    try {
      ...
      if (!user) {
        logUnauthorizedAccess(request, { user: { id: session.user.id } }, 'User not found in database');
        return NextResponse.json(
      ...
      if (!user.trang_thai) {
        logUnauthorizedAccess(request, { user: { id: session.user.id } }, 'User account is inactive');
        return NextResponse.json(
      ...
      session.user.trang_thai = user.trang_thai;
      session.user.vai_tro = user.vai_tro;

    } catch (error) {
      console.error('Error validating user in requireAuth middleware:', error);
      logUnauthorizedAccess(request, { user: { id: session.user.id } }, 'Database error during user validation');
      return NextResponse.json(
