              ...
              <CardHeader className="pb-3">
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    <CardTitle className="text-lg">{tenant.ho_ten}</CardTitle>
                    <div className="flex items-center gap-2 mt-1">
                      {tenant.phong ? (
                        <>
                          <Badge variant="outline" className="text-xs">
                            {tenant.phong.ma_phong}
                          </Badge>
                          {/* tên phòng */}
                          <Badge variant="outline" className="text-xs">
                            {tenant.phong.ten_phong}
                          </Badge>
                          <Badge className="text-xs bg-emerald-100 text-emerald-700 hover:bg-emerald-100 border-none">
                            Đang thuê
                          </Badge>
                        </>
                      ) : (
              ...
              <CardContent className="space-y-3">
                <div className="space-y-2 text-sm">
                  <div className="flex items-center gap-2">
                    ...
                    <span>{tenant.dien_thoai}</span>
                  </div>

                  {tenant.que_quan && tenant.que_quan !== "" ? (
                    <div className="flex items-center gap-2">
                      <MapPin className="h-3 w-3 text-muted-foreground" />
                      <span className="truncate">{tenant.que_quan}</span>
                    </div>
                  ) : (
                    <div className="flex items-center gap-2">
                      <MapPin className="h-3 w-3 text-muted-foreground" />
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span>Vào: {formatDate(tenant.ngay_vao_o)}</span>
                  </div>

                  <div className="flex items-center gap-2">
                    <DollarSign className="h-3 w-3 text-muted-foreground" />
                    <span>Cọc: {formatCurrency(tenant.tien_coc)}</span>
                  </div>
                </div>
                  ...
                  <div className="flex flex-col sm:flex-row gap-2">
                    ...
                    <Button
                      ...
                      onClick={(e) => {
                        e.stopPropagation();
                        if (tenant.phong) {
                          window.location.href = `/bills/create?roomId=${tenant.phong.id}`;
                        }
                      }}
                    >
                      Tạo hóa đơn
