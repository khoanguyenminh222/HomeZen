...
export class EmailConfigurationService {
  ...
  static async create(config, createdBy) {
    ...
    const existing = await prisma.cFG_EMAIL.findFirst({
    ...
    const encryptedHost = await encryptionService.encrypt(validatedConfig.smtp_host);
    const encryptedUser = await encryptionService.encrypt(validatedConfig.smtp_user);
    const encryptedPassword = await encryptionService.encrypt(validatedConfig.smtp_password);

    // Tạo cấu hình mới
    const emailConfig = await prisma.cFG_EMAIL.create({
      data: {
        smtp_host: encryptedHost,
        smtp_port: validatedConfig.smtp_port,
        smtp_user: encryptedUser,
        smtp_password: encryptedPassword,
        ten_nguoi_gui: validatedConfig.ten_nguoi_gui,
        su_dung_tls: validatedConfig.su_dung_tls,
        su_dung_ssl: validatedConfig.su_dung_ssl,
        trang_thai: true,
        nguoi_tao: createdBy,
      },
    });
  ...
  static async update(id, config) {
    ...
    const existing = await prisma.cFG_EMAIL.findUnique({
      where: { id },
    });
    ...
    const testConfig = {
      smtp_host: config.smtp_host || await encryptionService.decrypt(existing.smtp_host),
      smtp_port: config.smtp_port || existing.smtp_port,
      smtp_user: config.smtp_user || await encryptionService.decrypt(existing.smtp_user),
      smtp_password: config.smtp_password || await encryptionService.decrypt(existing.smtp_password),
      ten_nguoi_gui: config.ten_nguoi_gui || existing.ten_nguoi_gui,
      su_dung_tls: config.su_dung_tls !== undefined ? config.su_dung_tls : existing.su_dung_tls,
      su_dung_ssl: config.su_dung_ssl !== undefined ? config.su_dung_ssl : existing.su_dung_ssl,
    };
    ...
    if (config.smtp_host) {
      updateData.smtp_host = await encryptionService.encrypt(config.smtp_host);
    }
    if (config.smtp_port !== undefined) {
      updateData.smtp_port = config.smtp_port;
    }
    if (config.smtp_user) {
      updateData.smtp_user = await encryptionService.encrypt(config.smtp_user);
    }
    if (config.smtp_password) {
      updateData.smtp_password = await encryptionService.encrypt(config.smtp_password);
    }
    if (config.ten_nguoi_gui !== undefined) {
      updateData.ten_nguoi_gui = config.ten_nguoi_gui;
    }
    if (config.su_dung_tls !== undefined) {
      updateData.su_dung_tls = config.su_dung_tls;
    }
    if (config.su_dung_ssl !== undefined) {
      updateData.su_dung_ssl = config.su_dung_ssl;
    }

    // Cập nhật
    const updated = await prisma.cFG_EMAIL.update({
      where: { id },
      data: updateData,
    });
   ...
   */
  static async get() {
    const config = await prisma.cFG_EMAIL.findFirst({
      where: { trang_thai: true },
  ...
  static async getDecrypted() {
    ...
    const smtpUser = await encryptionService.decrypt(config.smtp_user);
    return {
      id: config.id,
      smtp_host: await encryptionService.decrypt(config.smtp_host),
      smtp_port: config.smtp_port,
      smtp_user: smtpUser,
      smtp_password: await encryptionService.decrypt(config.smtp_password),
      ten_nguoi_gui: config.ten_nguoi_gui,
      su_dung_tls: config.su_dung_tls,
      su_dung_ssl: config.su_dung_ssl,
  ...
  static async delete(id) {
    const config = await prisma.cFG_EMAIL.findUnique({
      where: { id },
    });
  ...
  static async testConnection(config) {
    try {
      ...
      let smtpHost = validatedConfig.smtp_host.trim();
      ...
      const transporter = nodemailer.createTransport({
        ...
        secure: validatedConfig.su_dung_ssl, // true for 465, false for other ports
        auth: {
          user: validatedConfig.smtp_user,
          pass: validatedConfig.smtp_password,
        },
        tls: {
          rejectUnauthorized: validatedConfig.su_dung_tls,
        },
      });
  ...
  static async sendTestEmail(config, testEmail) {
    try {
      ...
      const fromEmail = validatedConfig.smtp_user;
      ...
      // Gửi email test
      const fromName = validatedConfig.ten_nguoi_gui || fromEmail;
      const mailOptions = {
        from: validatedConfig.ten_nguoi_gui
          ? `${validatedConfig.ten_nguoi_gui} <${fromEmail}>`
          : fromEmail,
        to: testEmail,
          ...
          <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
            ...
            <div style="background-color: #f5f5f5; padding: 15px; border-radius: 5px; margin: 20px 0;">
              ...
              <ul style="margin: 10px 0; padding-left: 20px;">
                ...
                <li>SMTP Port: ${validatedConfig.smtp_port}</li>
...
Thông tin cấu hình:
- SMTP Host: ${smtpHost}
- SMTP Port: ${validatedConfig.smtp_port}
...
Email này được gửi tự động từ hệ thống ${brandName} để kiểm tra cấu hình email.
  ...
  static formatResponse(config) {
    return {
      id: config.id,
      smtp_host: config.smtp_host, // Trả về encrypted, không decrypt
      smtp_port: config.smtp_port,
      smtp_user: config.smtp_user, // Trả về encrypted, không decrypt
      ten_nguoi_gui: config.ten_nguoi_gui,
      su_dung_tls: config.su_dung_tls,
      su_dung_ssl: config.su_dung_ssl,
      trang_thai: config.trang_thai,
      ngay_tao: config.ngay_tao,
      ngay_cap_nhat: config.ngay_cap_nhat,
      // Note: smtp_password không được trả về trong response
    };
  }
}
